...
let content = fs.readFileSync('src/core/domain/potentialSystem.ts', 'utf-8');

const target = `export function generateInitialPlayerStats(
  position: Position | '',
  seed: number,
...
const newFunc = `export function generateInitialPlayerStats(
  position: Position | '',
  seed: number,
  ...
  for (const d of dna) {
    profileScore += d.rarity === 'GOAT' ? 40 : d.rarity === 'LEGENDARY' ? 30 : d.rarity === 'EPIC' ? 20 : 10;
  }
...
const newContent = content.substring(0, content.indexOf(target)) + newFunc;

fs.writeFileSync('src/core/domain/potentialSystem.ts', newContent);
console.log('updated');
