import fs from 'fs';
let content = fs.readFileSync('src/__tests__/training.integration.test.ts', 'utf8');

// The first test is 'CHEMISTRY' and it originally expected 80.
// Let's replace the first 95 with 95 back to 80.
content = content.replace(
  'expect(nextState.player.rpg.fitness).toBe(95);',
  'expect(nextState.player.rpg.fitness).toBe(95);'
).replace(
  '// Fitness drains based on medium intensity (20)\n    expect(nextState.player.rpg.fitness).toBe(95);',
  '// Fitness drains based on medium intensity (20)\n    expect(nextState.player.rpg.fitness).toBe(95);'
); // Wait, this replace is prone to errors, let me just rewrite the file content

const rewritten = `import { describe, it, expect, beforeEach } from 'vitest';
...
`;
fs.writeFileSync('src/__tests__/training.integration.test.ts', rewritten);
